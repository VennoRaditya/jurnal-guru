        ...
        <div class="max-w-5xl mx-auto pb-20">
            ...
            <form action="{{ route('guru.presensi.storeJurnal') }}" method="POST" class="space-y-10">
                @csrf
                {{-- Hidden input kelas harus dipastikan terisi --}}
                <input type="hidden" name="kelas" value="{{ $kelas_nama }}">

                {{-- BAGIAN 1: JURNAL MATERI --}}
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-3">
                                ...
                                <input type="text" name="judul_materi" required value="{{ old('judul_materi') }}" placeholder="Contoh: Pemrograman Berorientasi Objek"
                                    class="w-full bg-slate-50/50 border-2 border-transparent rounded-3x1 px-7 py-5 text-sm font-black text-slate-700 focus:bg-white focus:border-blue-500/20 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none">
                            </div>
                ...
                <div class="bg-white rounded-[3rem] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] overflow-hidden transition-all">
                        ...
                        <div class="flex items-center gap-5">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">
                                    Terdeteksi: {{ count($siswas) }} Siswa di Kelas {{ $kelas_nama }}
                                </p>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr>
                                    <td colspan="2" class="py-32 text-center">
                                        <div class="flex flex-col items-center justify-center space-y-4">
                                            ...
                                            <p class="font-black text-slate-400 uppercase tracking-widest text-xs">Siswa Tidak Ditemukan</p>
                                            <p class="text-slate-400 text-[10px] max-w-xs mx-auto">Siswa untuk kelas "{{ $kelas_nama }}" belum terdaftar di database.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
                    ...
                    <div class="p-8 md:p-12 bg-slate-50/50 flex justify-end border-t border-slate-50">
                        <button type="submit" @if($siswas->isEmpty()) disabled @endif 
                            class="group relative w-full md:w-auto overflow-hidden {{ $siswas->isEmpty() ? 'bg-slate-300' : 'bg-slate-900' }} text-white px-14 py-5 rounded-3x1 text-[11px] font-black uppercase tracking-[0.2em] transition-all hover:bg-blue-600 hover:shadow-2xl hover:shadow-blue-500/30 active:scale-95">
